...
public class DBUtils {
	...
	public static StaffInfo loadStaffInfo(String s) {
		if (s.equals("null")) {
			return null;
		}
		String info[] = s.split("@");
		...
		return new StaffInfo(info[0], info[1], info[2], info[3], Boolean.parseBoolean(info[4]), Integer.parseInt(info[5]), info[6]);
	}
	...
	public static String toStaffInfoString(StaffInfo info) {
		...
		return info.name() + "@" + info.type() + "@" + (info.company() == null ? "#null#" : info.company()) + "@" + (info.city() == null ? "#null#" : info.city()) + "@" + info.isFemale() + "@" + info.age() + "@" + info.phoneNumber();
	}
